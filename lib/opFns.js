...
/**
 * ********************************
 * solidity-coverage modifications
 * ----------- START --------------
 * ********************************
 */
...
function debug(msg){
  fs.appendFileSync('./scDebugLog', msg);
}
...
module.exports = {
  ...
  LOG: function (memOffset, memLength) {
    ...
    let notCoverageEvent = scTopics.findIndex(topic => topic === toWrite.topics[0]) === -1;

    if ( notCoverageEvent || process.env.NO_EVENTS_FILTER ){
      subGas(runState, new BN(runState._common.param('gasPrices', 'logTopic')).imuln(numOfTopics).iadd(new BN(runState._common.param('gasPrices', 'logData')).imuln(memLength)))
