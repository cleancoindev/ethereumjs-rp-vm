...
module.exports = {
  ...
  setup: function (opts) {
    ...
    if (opts.debugTopics) {
      this.debugTopics = fs.readFileSync(opts.debugTopics).toString().split('\n')
      this.debugTopics.pop()
    }

    this.dumpLogs = opts.dumpLogs
  },
  ...
  LOG: function (memOffset, memLength) {
    ...
    const topicsHex = topics.map(function (x) { return x.toString('hex') })

    // log dump if configured
    if (this.dumpLogs) {
      const logEntry = {
        address: runState.address.toString('hex'),
        topics: topicsHex,
        data: mem.toString('hex')
      }

      // TODO!! Write all logs at the end of execution (not in each op)
      fs.appendFileSync(this.dumpLogs, JSON.stringify(logEntry) + '\n')
    }

    // ignore debug events in runState
    let notDebug = this.debugTopics.findIndex(topic => topic === topicsHex[0]) === -1
    // TODO check if ENV variable is redundant
    if (notDebug || process.env.NO_EVENTS_FILTER) {
      runState.logs.push([
        runState.address,
        topics,
        mem
