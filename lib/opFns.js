...
function setup(opts) {
  ...
  if (opts.debugTopics) {
    debugTopics = fs.readFileSync(opts.debugTopics).toString().split('\n');
    debugTopics.pop();
  }

  dumpLogs = opts.dumpLogs;
}
...
module.exports = {
  ...
  LOG: function (memOffset, memLength) {
    ...
    if (dumpFile) {
      ...
      fs.appendFileSync(dumpLogs, JSON.stringify(logEntry) + '\n');
    }

    // ignore debug events in runState
    let notDebug = scTopics.findIndex(topic => topic === toWrite.topics[0]) === -1;
